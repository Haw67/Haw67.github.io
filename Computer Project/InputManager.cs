    ...
    void Update()
    {
        if (Input.touchCount == 1)
        {
            ...
            if (isRotating && Input.GetTouch(0).phase == TouchPhase.Moved)
            {
                ...
                float newXRotation = selectedFurniture.transform.eulerAngles.x - delta.y * rotationSpeed;
                newXRotation = Mathf.Clamp(newXRotation, 0f, 80f);
                selectedFurniture.transform.eulerAngles = new Vector3(newXRotation, selectedFurniture.transform.eulerAngles.y, selectedFurniture.transform.eulerAngles.z);
                initialTouchPosition = touchPosition;
